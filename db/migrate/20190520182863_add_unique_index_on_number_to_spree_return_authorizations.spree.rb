...
class AddUniqueIndexOnNumberToSpreeReturnAuthorizations < ActiveRecord::Migration[
    5.0
]
    def change
        unless index_exists?(
            ...
            numbers =
                Spree::ReturnAuthorization.group(:number).having('sum(1) > 1')
                    .pluck(:number)
            authorizations = Spree::ReturnAuthorization.where(number: numbers)

            authorizations.find_each do |authorization|
                authorization.number =
                    authorization.class.number_generator.method(
                        :generate_permalink
                    )
                        .call(authorization.class)
                authorization.save
            end
