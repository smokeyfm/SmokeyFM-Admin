...
class SplitPricesFromVariants < ActiveRecord::Migration[4.2]
    ...
    def down
        ...
        add_column :spree_variants,
                   :price,
                   :decimal,
                   after: :sku, scale: 2, precision: 8

        prices.each do |price|
            ApplicationRecord.connection.execute(
                "update spree_variants set price = #{
                    price['amount']
                } where id = #{price['variant_id']}"
            )
        end

        change_column :spree_variants,
                      :price,
                      :decimal,
                      after: :sku, scale: 2, precision: 8, null: false
