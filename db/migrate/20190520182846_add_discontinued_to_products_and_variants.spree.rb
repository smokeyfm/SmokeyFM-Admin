...
We will print out a report of the data we are fixing now: "
        ...
        Spree::Product.only_deleted.each do |product|
            ...
            the_dup =
                Spree::Product.find_by(slug: product.slug)
            if the_dup.nil?
                # check to see if there are line items attached to any variants
                if Spree::Variant.with_deleted.where(product_id: product.id)
                     .map(&:line_items)
                     .any?
                    puts "recovering deleted product id #{
                             product.id
                         } ... this will un-delete the record and set it to be discontinued"
                    ...
                    puts "leaving product id #{
                             product.id
                         } deleted because there are no line items attached to it..."
                end
            else
                puts "leaving product id #{
                         product.id
                     } deleted because there is a duplicate slug for '#{
                         product.slug
                     }' (product id #{the_dup.id}) "
                if product.variants.map(&:line_items).any?
                    puts "WARNING: You may still have bugs with product id #{
                             product.id
                         } (#{
                             product.name
                         }) until you run rake db:fix_orphan_line_items"
                end
            end
        end
        ...
        Spree::Variant.only_deleted.each do |variant|
            # determine if there is a slug duplicate
            the_dup =
                Spree::Variant.find_by(sku: variant.sku)
            if the_dup.nil?
                # check to see if there are line items attached to any variants
                if variant.line_items.any?
                    puts "recovering deleted variant id #{
                             variant.id
                         } ... this will un-delete the record and set it to be discontinued"
                ...
                puts "leaving variant id #{
                         variant.id
                     } deleted because there is a duplicate SKU for '#{
                         variant.sku
                     }' (variant id #{the_dup.id}) "
                if variant.line_items.any?
                    puts "WARNING: You may still have bugs with variant id #{
                             variant.id
                         } (#{
                             variant.name
                         }) until you run rake db:fix_orphan_line_items"
