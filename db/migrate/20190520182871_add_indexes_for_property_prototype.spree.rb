...
class AddIndexesForPropertyPrototype < ActiveRecord::Migration[5.0]
    def change
        duplicates =
            Spree::PropertyPrototype.group(:prototype_id, :property_id).having(
                'sum(1) > 1'
            )
                .size
        ...
        duplicates.each do |f|
            ...
            prototypes =
                Spree::PropertyPrototype.where(
                    prototype_id: prototype_id, property_id: property_id
                )
                    .last(count)
            prototypes.map(&:destroy)
        end

        if index_exists? :spree_property_prototypes,
                       %i[prototype_id property_id]
            remove_index :spree_property_prototypes,
                         %i[prototype_id property_id]
