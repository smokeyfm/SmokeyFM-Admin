...
class AddMissingIndexesOnSpreeTables < ActiveRecord::Migration[4.2]
    def change
        if data_source_exists?(:spree_promotion_rules_users) &&
             !index_exists?(
        ...
        if data_source_exists?(:spree_products_promotion_rules) &&
             !index_exists?(
        ...
        if data_source_exists?(:spree_orders_promotions) &&
             !index_exists?(:spree_orders_promotions, %i[promotion_id order_id])
            add_index :spree_orders_promotions, %i[promotion_id order_id]
        end

        if data_source_exists?(:spree_properties_prototypes) &&
             !index_exists?(:spree_properties_prototypes, :prototype_id)
        ...
        if data_source_exists?(:spree_properties_prototypes) &&
            ...
            add_index :spree_properties_prototypes,
                      %i[prototype_id property_id],
                      name:
                          'index_properties_prototypes_on_prototype_and_property'
        end

        if data_source_exists?(:spree_taxons_prototypes) &&
             !index_exists?(:spree_taxons_prototypes, %i[prototype_id taxon_id])
            add_index :spree_taxons_prototypes, %i[prototype_id taxon_id]
        end

        if data_source_exists?(:spree_option_types_prototypes) &&
             !index_exists?(:spree_option_types_prototypes, :prototype_id)
        ...
        if data_source_exists?(:spree_option_types_prototypes) &&
             !index_exists?(
                 ...
                 name:
                     'index_option_types_prototypes_on_prototype_and_option_type'
             )
        ...
        if data_source_exists?(:spree_option_values_variants) &&
             !index_exists?(
