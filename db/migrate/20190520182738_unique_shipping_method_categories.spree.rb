...
class UniqueShippingMethodCategories < ActiveRecord::Migration[4.2]
    def change
        ...
        duplicates =
            klass.select((columns + %w[COUNT(*)]).join(',')).group(
                columns.join(',')
            )
                .having('COUNT(*) > 1')
        ...
        add_index klass.table_name,
                  columns,
                  unique: true, name: 'unique_spree_shipping_method_categories'

        say "Remove redundant simple index on #{klass.table_name}"
        remove_index klass.table_name,
                     name:
