...
class UpgradeAdjustments < ActiveRecord::Migration[4.2]
    def up
        ...
        Spree::Adjustment.where(source_type: 'Spree::Shipment')
        ...
        Spree::Adjustment.where(originator_type: 'Spree::TaxRate')
            .find_each do |adjustment|
        ...
        Spree::Adjustment.where(originator_type: 'Spree::PromotionAction')
            .find_each do |adjustment|
            next if adjustment.originator.nil?
            adjustment.source = adjustment.originator
            begin
                if adjustment.source.calculator_type ==
